...
> ⚠️ **注意**：下面的 `source ... activate` 路径仅为示例，请务必将其中的 `/root/.qwenpaw/workspaces/default/skills/auto-conference-minutes/scripts/auto_minutes_python/.venv` 替换为你本机上 `.venv` 的**真实绝对路径**（即 `auto-conference-minutes/scripts/auto_minutes_python/.venv` 在你机器上的实际位置）。如尚未创建该虚拟环境，请先进入 `scripts/auto_minutes_python/` 目录执行 `uv venv .venv` 并安装依赖。

```bash
# 请将下面的路径替换为你本机 .venv 的真实绝对路径
